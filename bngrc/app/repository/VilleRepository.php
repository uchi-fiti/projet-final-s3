...
class VilleRepository {
    ...
    public function getVilleDetailedInfo()
    {
        $stmt = $this->pdo->query("
            ...
            FROM bngrc_villes v
            LEFT JOIN bngrc_besoins b ON b.ville_id = v.id
    ...
    public function getDashboardStats()
    {
        ...
        $stmt = $this->pdo->query("SELECT COALESCE(SUM(quantite * prix_unitaire), 0) AS total FROM bngrc_besoins");
        $totalBesoins = $stmt->fetch(PDO::FETCH_ASSOC)['total'];

        // Total dons (valeur monétaire)
        $stmt = $this->pdo->query("SELECT COALESCE(SUM(montant), 0) AS total FROM bngrc_dons");
